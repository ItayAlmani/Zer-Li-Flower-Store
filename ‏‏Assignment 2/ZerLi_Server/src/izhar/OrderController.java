...
public class OrderController extends ParentController implements IOrder {
	...
	private BigInteger getNextID(Order order) throws Exception {
		...
		else if (order.getDeliveryType() != null && order.getDeliveryType().equals(DeliveryType.Shipment)
	...
	public ArrayList<Object> handleGet(ArrayList<Object> obj) {
		...
		for (int i = 0; i < obj.size(); i += 11) {
			BigInteger deliveryID = obj.get(i + 2) == null ? null
			...
			ords.add(parse(BigInteger.valueOf(Long.valueOf((int) obj.get(i))),
					BigInteger.valueOf(Long.valueOf((int) obj.get(i + 1))), deliveryID,
					obj.get(i + 3) == null ? null : (String) obj.get(i + 3), shipmentID,
					obj.get(i + 5) == null ? null : (String) obj.get(i + 5), (String) obj.get(i + 6),
					(String) obj.get(i + 7), (String) obj.get(i + 8), (Timestamp) obj.get(i + 9),
					(float) obj.get(i + 10)));
		}
		return ords;
	}
	...
	public Order parse(BigInteger orderID, BigInteger customerID, BigInteger deliveryID, String payMethod,
		...
		if (deliveryType == null)
			return new Order(orderID, customerID, OrderType.valueOf(type),
					payMethod == null ? null : PayMethod.valueOf(payMethod), greeting, OrderStatus.valueOf(orderStatus),
					ldtDate, price);
		else if (DeliveryType.valueOf(deliveryType).equals(DeliveryType.Pickup))
			return new Order(orderID, customerID, new DeliveryDetails(deliveryID), OrderType.valueOf(type),
					PayMethod.valueOf(payMethod), greeting, DeliveryType.valueOf(deliveryType),
					OrderStatus.valueOf(orderStatus), ldtDate, price);
		else if (DeliveryType.valueOf(deliveryType).equals(DeliveryType.Shipment))
			return new Order(orderID, customerID, new ShipmentDetails(shipmentID), OrderType.valueOf(type),
					PayMethod.valueOf(payMethod), greeting, DeliveryType.valueOf(deliveryType),
					OrderStatus.valueOf(orderStatus), ldtDate, price);
		else
			return null;
